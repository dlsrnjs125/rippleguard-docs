...
Local LLM Agent Run은 `modelProvider`, `modelName`, `sourceRevision`, `artifactDigest`, `providerManifestDigest`, `modelfileDigest`, `quantization`, `runtimeVersion`, `promptVersion`, `toolVersion`, `contextTokenCount`, `inferenceLatencyMs`, `retryCount`와 `outputSchemaVersion`을 Audit 가능한 Metadata로 남긴다. Chain-of-Thought, 전체 Prompt와 전체 Response는 Evaluation Run 또는 Audit Event에 저장하지 않는다.

...
| Governance Service | Audit Replay Service | Kafka | `agent.evaluation.requested.v1` | Phase 2 Agent Runtime request event and Phase 1 historical request trace | RUNTIME_VERIFIED_PHASE2_HAPPY_PATH |
| Governance Service | Audit Replay Service | Kafka | `agent.evaluation.completed.v1` | Phase 1 Mock Evaluation result; not the normal Phase 2 result path | RUNTIME_VERIFIED_FOR_PHASE1 |
| Governance Service | Agent Runtime | REST API | Agent Request Contract | Fixed Snapshot and model-version Loan Proposal request | RUNTIME_VERIFIED_PHASE2_HAPPY_PATH |
| Agent Runtime | Governance Service | REST API | Loan Proposal / Phase 2 Decision Envelope | Validated proposal or explicit failure response; not final Loan state mutation | RUNTIME_VERIFIED_PHASE2_HAPPY_PATH |
| Governance Service | Audit Replay Service | Kafka | `governance.agent-result.validated.v1` | Governance-validated Agent result recorded after output validation | RUNTIME_VERIFIED_PHASE2_HAPPY_PATH |
...
- `RUNTIME_VERIFIED_PHASE2_HAPPY_PATH`: Producer and consumer implementation exist and Phase 2 happy path E2E passed; required failure drills may still be blocked.

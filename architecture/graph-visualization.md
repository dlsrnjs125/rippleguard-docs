...
| 데이터 Source | Topology Manifest | Audit Read Model |
...
Execution Graph 링크는 `correlationId`, `causationId`, `evaluationRunId`, `agentRunId`를 중심으로 구성한다. 실제 Event 이동은 선택된 링크 또는 재생 중인 링크에만 방향성 Particle Animation을 적용한다. 모든 링크에 계속 Particle을 표시하지 않는다.
...
| `rippleguard-contracts` | 향후 Graph Node·Link DTO와 조회 API 계약 |
| `rippleguard-infra` | 실제 서비스·Topic·Policy 배치 기준, Topology Manifest 검증 근거 |
...
| `rippleguard-governance-service` | Agent Registry와 EvaluationRun Reference 제공 |

## 데이터 Source 원칙

### Architecture Graph

Phase 6에서는 `rippleguard-web`이 Versioned Topology Manifest를 이용해 렌더링한다. 이 Manifest는 발표용 임의 데이터가 아니라 다음 기준과 대조할 수 있어야 한다.

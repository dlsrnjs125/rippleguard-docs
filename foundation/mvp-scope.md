...
- XGBoost 또는 LightGBM 기반 Loan Decision Agent와 SHAP 설명
- Ollama 기반 Local LLM Runtime
- Local LLM Provider Adapter
- 공유 Foundation Model 기반 RippleGuard Agent와 Evidence & Control Agent 독립 실행
- Model Manifest와 Version Metadata
- Structured Output Validation
- Local LLM 실패 시 안전 축소
...
- 외부 LLM API 필수 의존
- Agent별 별도 대형 모델
- Fine-Tuning, Online Learning, 자동 재학습
- Multi-Model Router
- Model Weight Git 저장
- Chain-of-Thought 저장·표시
- 운영 GPU Cluster
- 복수 금융기관 또는 복수 대출상품
...
외부 LLM API는 MVP의 필수 실행 경로가 아니며, 후속 품질 Benchmark와 Provider Adapter 검증 후보다.

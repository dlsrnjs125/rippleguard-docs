...
- Versioned Topology Manifest의 원본 소유 Repository는 `rippleguard-infra`로 둔다.
- Graph 계약은 Phase 6에서 공통 Node·Link Core DTO와 Static Manifest Schema를 먼저 정의하고, Phase 7에서 Execution Graph API와 Trace Edge Extension으로 확장한다.
- Execution Graph Edge는 실제 Event causation, Reference 기반 관계, 파생 관계, Static Topology 관계를 구분한다.

...
현재 **Phase 0 — Foundation & Architecture Baseline**은 `VERIFIED`이고 **Phase 1 — Core MSA Foundation**은 `IN_REVIEW`이다. Phase 1 구현 PR과 CI는 확인됐지만 image provenance와 Docker Compose runtime verification이 완료되지 않아 `VERIFIED`로 전환하지 않는다. 상세 진척과 완료 검증은 [Phase 1 progress](phases/phase-01-core-msa/progress.md)와 [verification](phases/phase-01-core-msa/verification.md)에서 추적한다.
